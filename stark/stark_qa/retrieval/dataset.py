...
class STaRKDataset:
    ...
    def _download(self):
        ...
        """
        # If dataset_root is already set and exists, skip download
        if self.dataset_root and osp.exists(self.dataset_root):
            print(f"Dataset {self.name} already exists at {self.dataset_root}, skipping download.")
            return

        # If root is provided but dataset doesn't exist, download it
        if self.root is not None:
            ...
            # If no root provided, get from HF cache without downloading
            self.dataset_root = download_hf_folder(
                STARK_QA_DATASET["repo"],
                osp.join(STARK_QA_DATASET["folder"], self.name),
                repo_type="dataset",
                save_as_folder=None,  # Use cache
            )
    ...
    def get_idx_split(self, test_ratio: float = 1.0) -> dict:
        ...
        for split in ['train', 'val', 'test', 'test-0.1']:
            ...
            if osp.exists(indices_file):
                try:
                    with open(indices_file, 'r') as f:
                        indices = f.read().strip().split('\n')
                    query_ids = [int(idx) for idx in indices if idx.strip()]
                    # Filter out IDs that don't exist in our dataset
                    valid_query_ids = [qid for qid in query_ids if qid in self.indices]
                    if valid_query_ids:
                        split_idx[split] = torch.LongTensor([self.indices.index(query_id) for query_id in valid_query_ids])
                    else:
                        # Create empty tensor for missing splits
                        split_idx[split] = torch.LongTensor([])
                except (FileNotFoundError, ValueError) as e:
                    print(f"Warning: Could not load split file {indices_file}: {e}")
                    split_idx[split] = torch.LongTensor([])
            else:
                # Create empty tensor for missing splits
                split_idx[split] = torch.LongTensor([])
